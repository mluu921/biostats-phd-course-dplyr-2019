...
The dataframe in this example is structured in two different ways, in both a long and wide format. The wide format is structured by 'plot_id' in which each row is a single 'plot_id'. The long format is structured by 'mean_weight', in which each row pertains to a single 'mean_weight'. We can easily jump back and forth between these two different data structures using the `gather()` and `spread()` function.
